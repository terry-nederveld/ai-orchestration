...
describe('mapIssueToWorkItem', () => {
  it('maps the core fields, id, and browse url', () => {
    const issue: JiraIssue = {
      ...
      fields: {
        ...
        assignee: { name: 'ada', displayName: 'Ada Lovelace', emailAddress: 'ada@x.com' },
        updated: '2026-08-01T12:00:00.000+0000',
      },
    }

    const item = mapIssueToWorkItem(issue, 'https://jira.company.com')

    expect(item.id).toBe('jira-datacenter:PROJ-42')
    expect(item.provider).toBe('jira-datacenter')
    ...
    expect(item.assignees).toEqual([{ id: 'ada', displayName: 'Ada Lovelace', email: 'ada@x.com' }])
    expect(item.url).toBe('https://jira.company.com/browse/PROJ-42')
    expect(item.updatedAt).toEqual(new Date('2026-08-01T12:00:00.000+0000'))
  })

  it('carries the description through as a plain string, never ADF', () => {
    const issue: JiraIssue = {
      key: 'PROJ-1',
      fields: {
        summary: 'Has description',
        status: { id: '1', name: 'To Do' },
        description: 'This is plain wiki-markup text, *not* ADF JSON.',
      },
    }
    const item = mapIssueToWorkItem(issue, 'https://jira.company.com')
    expect(item.description).toBe('This is plain wiki-markup text, *not* ADF JSON.')
  })
  ...
  it('falls back to the assignee key when name is absent', () => {
    const issue: JiraIssue = {
      key: 'PROJ-5',
      fields: {
        summary: 'Assignee by key',
        status: { id: '1', name: 'To Do' },
        assignee: { key: 'ada-key' },
      },
    }
    const item = mapIssueToWorkItem(issue, 'https://jira.company.com')
    expect(item.assignees).toEqual([{ id: 'ada-key' }])
  })
