 ...
 * orchestration code never has to branch on Anthropic-specific status codes.
 */
...
/** Builds an OrchestratorError from a non-OK Anthropic HTTP response. */
export async function mapHttpErrorResponse(response: Response): Promise<OrchestratorError> {
  const bodyText = await response.text().catch(() => '')
  const message =
    describeErrorBody(bodyText) ?? `Anthropic API error: ${response.status} ${response.statusText}`
